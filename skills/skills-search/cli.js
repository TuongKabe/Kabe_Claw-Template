...
const API_BASE = 'https://skills.sh/api/search';

function fetchSkills(query = '', popular = false) {
...
Learn more: https://skills.sh
  ...
  try {
    const response = await fetchSkills(query, popular);
    ...
    results.forEach(skill => {
      ...
      if (showInstall) {
        console.log(`   Install: npx skills add ${skill.source}`);
      }
      console.log('');
    });
